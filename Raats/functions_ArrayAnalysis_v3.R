...
createQCPlots <- function(Data, factors, Table=NULL, normMeth="", postfix="", WIDTH=1000, HEIGHT=1414, POINTSIZE=24, MAXARRAY=41) {
...
# WIDTH, HEIGHT, POINTSIZE, MAXARRAY defaults for image settings; may have to be changed when having many samples
	...
	for (expF in factors) {
		...
		#create boxplot
		try(boxplotFun(Data, experimentFactor, plotColors, legendColors, normMeth, postfix_ext, WIDTH=WIDTH, HEIGHT=HEIGHT, POINTSIZE=POINTSIZE, MAXARRAY=MAXARRAY),TRUE)
		
		#create density plot
		try(densityFun(Data, plotColors, normMeth, postfix_ext, WIDTH=WIDTH, HEIGHT=HEIGHT, POINTSIZE=POINTSIZE, MAXARRAY=MAXARRAY),TRUE)
		
		#create cluster plot
		try(clusterFun(Data, experimentFactor, clusterOption1="pearson", clusterOption2="ward.D2", normMeth, postfix_ext, 
				  plotColors, legendColors, plotSymbols, legendSymbols, WIDTH=WIDTH, HEIGHT=HEIGHT, POINTSIZE=POINTSIZE, MAXARRAY=MAXARRAY),TRUE)

		#create heatmap (of array correlation)
		try(correlFun(Data, clusterOption1="pearson", clusterOption2="ward.D2", normMeth, postfix_ext, 
				 experimentFactor, legendColors, WIDTH=WIDTH, HEIGHT=HEIGHT, POINTSIZE=POINTSIZE, MAXARRAY=MAXARRAY),TRUE)

		#create PCA plot
		try(pcaFun(Data, experimentFactor, normMeth, postfix_ext, scaled_pca=TRUE, plotColors, legendColors,
			   plotSymbols, legendSymbols, namesInPlot=FALSE, WIDTH=WIDTH, HEIGHT=HEIGHT, POINTSIZE=POINTSIZE),TRUE)
			   
		try(pcaFun(Data, experimentFactor, normMeth, paste(postfix_ext,"2",sep=""), scaled_pca=TRUE, plotColors, legendColors,
			   plotSymbols, legendSymbols, namesInPlot=TRUE, WIDTH=WIDTH, HEIGHT=HEIGHT, POINTSIZE=POINTSIZE),TRUE)
...
saveStatOutputDESeq2 <- function(design,dds,annotation=NULL,na.rm=TRUE,includeIntercept=FALSE,createHists=TRUE,postfix="") {
  ...
  for (i in 1:length(colnames(design))) {
	if(includeIntercept | length(grep("intercept",colnames(design)[i],ignore.case=TRUE))==0) {
		cat(paste("--[[ Saving table for coefficient ", colnames(design)[i], " ]]--\n", sep="\t"))
		res <- results(dds, contrast=design[,i], format="GRanges")
